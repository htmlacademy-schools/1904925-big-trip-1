...
let siteEventsListElement = document.createElement("ul");
...
renderTemplate(siteFiltersElement, createSiteFilterTemplate(), RenderPosition.BEFOREEND);
renderTemplate(siteEventsElement, createSiteSortTemplate(), RenderPosition.BEFOREEND);

renderTemplate(siteEventsListElement, eventsItemWrapper(createEventEditTemplate()), RenderPosition.BEFOREEND)

for (let i = 0; i < 3; i++) {
    renderTemplate(siteEventsListElement, eventsItemWrapper(createEventItemTemplate()), RenderPosition.BEFOREEND)
}

renderTemplate(siteEventsElement, siteEventsListElement.outerHTML, RenderPosition.BEFOREEND);
